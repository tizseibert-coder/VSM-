...
**Erstellt:** 2026-09-14. Ursprünglich eine reine Planungssession auf Nutzerwunsch ("nur als Plan"); ab 2026-09-14/15 auf Nutzerfreigabe hin Schritt für Schritt umgesetzt.

**Stand 2026-09-15: alle 8 Schritte der Umsetzungsreihenfolge fertig ✅.** Migration geschrieben, aber **noch nicht auf einer echten Datenbank angewendet** (siehe Abschnitt "Testsystem, nicht Prod") — dafür weiterhin ausdrückliche Freigabe nötig, welches Projekt/welche Verbindung. Alles andere (Berechnungslogik, Kalenderpflege, Eingabepanel, Kapazitätsseite, Canvas-Badge, Szenario-Kopie, Glossar) ist geschrieben, getestet und läuft gegen einen vollständigen `next build`.
...
7. ✅ `createScenario` um `monthly_demand`/`shift_model` beim Kopieren ergänzt (`scenario-actions.ts`) — ohne diese zwei Felder in der expliziten Insert-Spaltenliste hätte jede neue Forecast-Revision (RF1/RF2/RF3) mit leerer CAMA-Ampel begonnen, obwohl der RF-Vergleich aus Schritt 0 genau auf dem Gegenteil aufbaut ("RF2 aus RF1 ableiten und Nachfrage anpassen", nicht bei null anfangen).
8. ✅ i18n + Glossar — zwei neue Begriffe in `glossary.ts`/`messages/{de,en}.json` (`cama`, `loadRate`), mit derselben strengen Testabdeckung wie jeder bestehende Begriff (kein verwaister Eintrag, keine leere Definition, `covers`-Test). `NEE` bekam bewusst **keinen** eigenen Glossareintrag — es taucht in der Oberfläche nirgends als eigenständiges Label auf (nur als erklärender Nebensatz "NEE kommen aus... OEE oben"), sondern ist Teil der `cama`-Definition selbst ("NEE — dieselbe Zahl wie OEE"). Tooltips verdrahtet auf der Kapazitätsseite: Spaltenköpfe "Load Rate" und "Ampel", in Übersichts- **und** Detailtabelle. Bewusst **nicht** auf den beiden Editor-Links ("Kapazitätsanalyse (CAMA)" in der Kopfzeile, "Kapazitätsdaten (CAMA) …" im Bearbeitungspanel) — `TermTooltip` rendert selbst einen `<button>`, das hätte dort einen Knopf in einem Link bzw. einem anderen Knopf verschachtelt (ungültiges HTML). Die Erklärung steht an diesen beiden Stellen stattdessen schon in Fliesstext (`capacityDataIntro`).

   Volle Suite: 478/478 grün (ein neuer Test: "covers the CAMA terms"), `tsc`/`eslint` sauber, `de.json`/`en.json` synchron, vollständiger `next build` erfolgreich.
